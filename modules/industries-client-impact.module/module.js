(function(){
  ...
  var body = root.querySelector('.impact__body');
  var current = 0;
  var isPaused = false;
  var timer = null;
  var INTERVAL = 15000;
  ...
  // Tap/click on the paragraph area toggles the hidden content for active slide
  if (body) {
    body.addEventListener('click', function (e) {
      var target = e.target;
      if (!target) return;
      // Only toggle when clicking description/details area (not nav buttons/links)
      var content = target.closest && target.closest('.impact__content');
      if (!content) return;
      if (target.closest('a, button')) return;

      var activeItem = body.querySelector('[data-impact-item].is-active');
      if (!activeItem) return;
      activeItem.classList.toggle('is-open');
    });
  }
